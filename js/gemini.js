...
[0, 2, 5, 8]`;
    ...
    // ─── Story Generation ─────────────────────────────────────
    buildStoryPrompt(wordMeaningGroups) {
        // wordMeaningGroups: [{word, meaningId, definition, hint}, ...] (5 items)
        const wordList = wordMeaningGroups.map((wm, i) =>
            `${i+1}. "${wm.word}" = ${wm.hint} (${wm.definition.replace(/\s*\[.*?\]\s*|\s*\(🤖.*?\)\s*/g, '').trim()})`
        ).join('\n');

        return `Write a short, vivid story (3-4 sentences, B1 level English) using ALL these words naturally. Make it cinematic — the reader should SEE the scene in their mind. Keep other vocabulary simple.

Words:
${wordList}

Return ONLY valid JSON:
{"story":"...","turkish":"...","usedForms":["form1","form2","form3","form4","form5"]}

Rules:
- Use EXACT word or simple inflection (past tense, plural)
- Turkish = natural translation
- usedForms = exact forms used in story
- NO markdown, NO explanation`;
    },

    async generateStoriesForMeanings(meaningGroups, onProgress = null) {
        const apiKey = await this.getApiKey();
        if (!apiKey) throw new Error('API anahtarı bulunamadı');
        ...
        for (let i = 0; i < meaningGroups.length; i++) {
            const group = meaningGroups[i]; // Array of 5 {word, meaningId, definition, hint}
            const prompt = this.buildStoryPrompt(group);

            if (onProgress) onProgress(i + 1, meaningGroups.length);
            ...
            try {
                ...
                if (!response.ok) {
                    console.warn(`Story generation failed for group ${i+1}:`, await response.text());
                    continue;
                }
                ...
                results.push({
                    ...
                    targetWords: group.map((wm, idx) => ({
                        word: wm.word,
                        meaningId: wm.meaningId,
                        hint: wm.hint,
                        usedForm: parsed.usedForms?.[idx] || wm.word
                    }))
                });
            } catch (e) {
                console.error(`Story generation error for group ${i+1}:`, e);
                continue;
            }

            // Small delay to avoid rate limiting
            if (i < meaningGroups.length - 1) {
